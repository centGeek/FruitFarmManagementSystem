...
export const ExpenseForm = ({ expense, onSave, onCancel, isLoading, sectors }: any) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
                <label htmlFor="type" className="block text-sm font-medium text-gray-700 mb-2">{t('form.typeLabel')} *</label>
            ...
            <div>
                <label htmlFor="sectorId" className="block text-sm font-medium text-gray-700 mb-2">{t('form.sectorLabel')}</label>
                ...
                {selectedSector && <div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded text-xs text-blue-800">{t('form.sectorAssignInfo')} <strong>{selectedSector.description || t('sectorFallback', { id: selectedSector.id })}</strong></div>}
            </div>
            <div>
            ...
            <div className="flex items-center pt-2">
                ...
                <label htmlFor="paid" className="ml-2 text-sm text-gray-700 font-medium">{t('form.paidLabel')}</label>
            </div>
            <div className="flex space-x-3 pt-6 border-t border-gray-100">
                ...
                <button type="button" onClick={onCancel} className="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-800 py-3 px-4 rounded-xl font-semibold transition-colors" disabled={isLoading}>{t('form.cancel')}</button>
            </div>
        </form>
    );
};
            ...
            <div className="flex items-start justify-between mb-4">
                <div className="flex items-center space-x-3">
                    ...
                    <div className="flex-1">
                        <h3 className="text-xl font-bold text-gray-800">{t('card.amountWithCurrency', { amount: formatCurrency(expense.amount) })}</h3>
                        <p className="text-sm text-gray-500">{expenseDate}</p>
                    </div>
                </div>
                <div className="flex items-center space-x-2">
                    <button onClick={() => onEdit(expense)} className="p-2 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg transition-colors text-base" title={t('card.editTitle')}>✏️</button>
                    <button onClick={() => onDelete(expense.id)} className="p-2 bg-gray-100 text-gray-600 hover:bg-gray-200 rounded-lg transition-colors text-base" title={t('card.deleteTitle')}>🗑️</button>
                </div>
            </div>
            <div className="mb-4 space-x-2">
                <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${typeDetails.color}`}>{typeLabel}</span>
                {isPaid ? <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-700">{t('card.paid')}</span> : <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-red-100 text-red-700">{t('card.unpaid')}</span>}
            </div>
            {assignedSector && (
                <div className="mb-3 p-2 bg-blue-50 border border-blue-200 rounded-lg">
                    <p className="text-xs text-blue-600 font-medium">{t('card.sector', { value: assignedSector.description || t('sectorFallback', { id: assignedSector.id }) })}{assignedSector.plantType && ` (${assignedSector.plantType})`}</p>
                </div>
            )}
            <div className="pt-4 border-t border-gray-100">
                <p className="text-sm font-medium text-gray-500 uppercase mb-1">{t('card.descriptionLabel')}</p>
                <p className="text-base text-gray-900 line-clamp-2">{expense.description || t('card.noDescription')}</p>
            </div>
        </div>
    );
};
