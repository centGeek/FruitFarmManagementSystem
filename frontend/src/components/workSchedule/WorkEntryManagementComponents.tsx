        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className={`bg-white rounded-2xl shadow-2xl ${sizeClasses[size]} w-full max-h-[90vh] overflow-y-auto`}>
                <div className="sticky top-0 bg-gradient-to-r from-green-50 to-lime-50 border-b border-green-200 px-6 py-4 rounded-t-2xl z-10 flex items-center justify-between">
                    <h2 className="text-xl font-bold text-gray-800 flex items-center"><Clock className="mr-2 text-green-600" size={24} />{title}</h2>
                    <button onClick={onClose} className="p-2 hover:bg-green-100 rounded-xl transition-colors"><X size={24} /></button>
                </div>
...
export const StatCard = ({ icon: Icon, count, label, color, isCurrency = false }: any) => {
    const colorMap: any = {
        green: 'from-green-100 to-green-200 text-green-600', blue: 'from-blue-100 to-blue-200 text-blue-600',
        amber: 'from-amber-100 to-amber-200 text-amber-600', purple: 'from-purple-100 to-purple-200 text-purple-600',
        red: 'from-red-100 to-red-200 text-red-600', indigo: 'from-indigo-100 to-indigo-200 text-indigo-600'
    };
    return (
        <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-200 hover:shadow-xl transition-shadow">
            <div className="flex items-center space-x-4">
                <div className={`w-14 h-14 bg-gradient-to-br ${colorMap[color]} rounded-2xl flex items-center justify-center`}><Icon size={28} /></div>
                <div>
                    <p className="text-3xl font-extrabold text-gray-900">{isCurrency ? `${parseFloat(count).toFixed(2)} zł` : count}</p>
...
export const DailyWorkForm = ({ date, employees, sectors, onSave, onCancel, isLoading }: any) => {
    const [entries, setEntries] = useState(() => employees.map((emp: any) => ({
        employeeId: emp.id, employee: emp, hours: '', kilogramsPicked: '', sectorId: '', workType: '', description: '',
        isPaidHourly: emp.workDetails?.isPaidHourly !== false
    })));

    const updateEntry = useCallback((id: number, field: string, value: any) => setEntries((p: any) => p.map((e: any) => e.employeeId === id ? { ...e, [field]: value } : e)), []);
    const quickFillAll = (field: string, value: any) => setEntries((p: any) => p.map((e: any) => ({ ...e, [field]: value })));
    const handleSubmit = () => {
        ...
        if (valid.length === 0) { onCancel('warning', 'Podaj wymagane dane dla co najmniej jednego pracownika'); return; }
        onSave(valid);
    };
    ...
    return (
        <div className="space-y-4">
            <div className="bg-green-50 border border-green-200 rounded-xl p-4"><p className="text-lg font-bold text-green-800">📅 {new Date(date).toLocaleDateString('pl-PL', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</p></div>
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                <p className="text-sm font-bold text-blue-800 mb-3">⚡ Szybkie wypełnienie dla wszystkich:</p>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div><label className="text-xs text-gray-600 mb-1 block">Godziny</label><div className="flex gap-2">{[4, 6, 8, 10, 11, 12].map(h => <button key={h} onClick={() => quickFillAll('hours', h.toString())} className="flex-1 bg-white hover:bg-blue-100 border border-blue-300 rounded-lg py-2 text-sm font-bold transition-colors">{h}h</button>)}</div></div>
                    <div><label className="text-xs text-gray-600 mb-1 block">Sektor</label><select onChange={(e) => quickFillAll('sectorId', e.target.value)} className="w-full px-3 py-2 border border-blue-300 rounded-lg text-sm"><option value="">Wybierz...</option>{sectors.map((s: any) => <option key={s.id} value={s.id}>{s.description}</option>)}</select></div>
                    <div className="md:col-span-2"><label className="text-xs text-gray-600 mb-1 block">Typ pracy</label><select onChange={(e) => quickFillAll('workType', e.target.value)} className="w-full px-3 py-2 border border-blue-300 rounded-lg text-sm"><option value="">Wybierz...</option>{WORK_TYPE_OPTIONS.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}</select></div>
                </div>
            </div>
                    ...
                    return (
                        <div key={entry.employeeId} className="bg-white border-2 border-gray-200 rounded-xl p-4 hover:border-green-300 transition-colors">
                            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
                                <div className="md:col-span-3"><p className="font-bold text-gray-900">{entry.employee.name} {entry.employee.surname}</p>{entry.employee.nickname && <p className="text-xs text-gray-500 italic">"{entry.employee.nickname}"</p>}</div>
                                {entry.employee.workDetails?.isPaidHourly ? (
                                    <div className="md:col-span-2"><label className="text-xs text-gray-500 mb-1 block">Godziny *</label><input type="number" step="0.5" min="0" max="24" value={entry.hours} onChange={(e) => updateEntry(entry.employeeId, 'hours', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 text-lg font-bold" placeholder="8" /></div>
                                ) : (
                                    <><div className="md:col-span-1"><label className="text-xs text-gray-500 mb-1 block">Godziny *</label><input type="number" step="0.5" min="0" max="24" value={entry.hours} onChange={(e) => updateEntry(entry.employeeId, 'hours', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 text-lg font-bold" placeholder="8" /></div>
                                    <div className="md:col-span-1"><label className="text-xs text-gray-500 mb-1 block">Kilogramy *</label><input type="number" step="0.1" min="0" value={entry.kilogramsPicked} onChange={(e) => updateEntry(entry.employeeId, 'kilogramsPicked', e.target.value)} className="w-full px-3 py-2 border border-orange-300 rounded-lg focus:ring-2 focus:ring-orange-500 text-lg font-bold" placeholder="50" /></div></>
                                )}
                                <div className="md:col-span-3"><label className="text-xs text-gray-500 mb-1 block">Sektor</label><select value={entry.sectorId} onChange={(e) => updateEntry(entry.employeeId, 'sectorId', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"><option value="">Brak</option>{sectors.map((s: any) => <option key={s.id} value={s.id}>{s.description} ({s.plantType})</option>)}</select></div>
                                <div className="md:col-span-4"><label className="text-xs text-gray-500 mb-1 block">Typ pracy</label><select value={entry.workType} onChange={(e) => updateEntry(entry.employeeId, 'workType', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"><option value="">Brak</option>{WORK_TYPE_OPTIONS.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}</select></div>
                            </div>
                            {(entry.hours || entry.kilogramsPicked) && <div className="mt-3 pt-3 border-t border-gray-100"><input type="text" value={entry.description} onChange={(e) => updateEntry(entry.employeeId, 'description', e.target.value)} className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-green-500 text-sm" placeholder="Uwagi (opcjonalnie)..." /></div>}
                        </div>
                    );
                })}
            </div>
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-4"><p className="text-sm text-gray-600 mb-1">Pracowników z danymi: <span className="font-bold text-green-600">{filledCount} / {employees.length}</span></p><p className="text-sm text-gray-600 mb-1">Suma godzin: <span className="font-bold text-blue-600">{totalHours.toFixed(1)}h</span></p>{totalKilograms > 0 && <p className="text-sm text-gray-600">Suma kilogramów: <span className="font-bold text-orange-600">{totalKilograms.toFixed(1)} kg</span></p>}</div>
            <div className="flex space-x-3 pt-2"><button onClick={handleSubmit} disabled={isLoading} className="flex-1 bg-green-600 hover:bg-green-700 text-white py-3 px-4 rounded-xl font-semibold transition-colors disabled:opacity-50 flex items-center justify-center shadow-md">{isLoading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin mr-2"></div> : <DollarSign className="mr-2" size={20} />} Zapisz wpisy</button><button onClick={() => onCancel()} disabled={isLoading} className="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-800 py-3 px-4 rounded-xl font-semibold transition-colors">Anuluj</button></div>
        </div>
    );
};
        ...
        <div onClick={onClick} className={`${paidClass} border-l-4 rounded-lg p-2 cursor-pointer hover:shadow-md transition-all text-xs mb-1`}>
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-1">
                    <button onClick={(e) => { e.stopPropagation(); onTogglePaid(entry.entryId); }} className={`p-1 rounded transition-colors text-sm ${entry.isPaid ? 'text-amber-700 hover:bg-amber-200' : 'text-emerald-700 hover:bg-emerald-200'}`} title={entry.isPaid ? 'Anuluj płatność' : 'Oznacz jako opłacone'}>{entry.isPaid ? '❌' : '💵'}</button>
                </div>
            </div>
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Wypłata zaliczki dla: ${employee.name} ${employee.surname}`} size="medium">
            <div className="space-y-4">
                ...
                <div><label className="text-sm font-medium text-gray-700 mb-2 block">Kwota zaliczki (zł) *</label><input type="number" step="0.01" min="0" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="np. 500.00" className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 text-lg font-bold" /></div>
                <div><label className="text-sm font-medium text-gray-700 mb-2 block">Opis (opcjonalnie)</label><textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="np. Zaliczka na poczet wypłaty tygodniowej..." rows={3} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500" /></div>
                <div className="mt-6 flex space-x-3">
                    <button onClick={handleSubmit} disabled={isLoading || !amount || parseFloat(amount) <= 0} className="flex-1 bg-green-600 hover:bg-green-700 text-white py-3 px-4 rounded-xl font-semibold transition-colors disabled:opacity-50 flex items-center justify-center shadow-md">{isLoading ? <Loader2 className="w-5 h-5 animate-spin mr-2" /> : <DollarSign className="mr-2" size={20} />}{isLoading ? 'Przetwarzanie...' : 'Potwierdź wypłatę zaliczki'}</button>
        ...
        <Modal isOpen={!!entry} onClose={onClose} title={`Szczegóły wpisu dla: ${entry.user?.name} ${entry.user?.surname}`} size="medium">
            <div className="space-y-4">
                ...
                <div className={`p-4 rounded-xl border ${entry.isPaid ? 'bg-emerald-50 border-emerald-300' : 'bg-amber-50 border-amber-300'}`}>
                    <div className="flex items-center justify-between">
                        <div className="mt-1"><p className="text-xs font-medium text-gray-600 uppercase mb-1">Status płatności</p><p className={`text-base font-bold ${entry.isPaid ? 'text-emerald-700' : 'text-amber-700'}`}>{entry.isPaid ? '✅ Zapłacono' : '🟡 Niezapłacono'}</p></div>
                        <div className="flex space-x-2"><button onClick={() => onTogglePaid(entry.entryId)} className={`px-4 py-2 rounded-lg font-medium transition-colors ${entry.isPaid ? 'bg-amber-100 text-amber-700 hover:bg-amber-200' : 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200'}`}>{entry.isPaid ? '❌ Oznacz jako niezapłacone' : '💵 Oznacz jako zapłacone'}</button></div>
                    </div>
