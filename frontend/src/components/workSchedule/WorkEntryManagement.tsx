...
export default function WorkEntryManagement() {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-green-50 via-lime-50 to-green-100 dark:from-gray-900 dark:via-gray-900 dark:to-gray-800 p-6 font-sans">
            <div className="max-w-7xl mx-auto">
                <header className="mb-8">
                    <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-100 mb-2 flex items-center"><Calendar className="text-green-600 dark:text-green-300 mr-3" size={40} /> {t("header.title")}</h1>
                        ...
                        <div className="space-y-4">
                             <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {editingEntry.user?.isPaidHourly !== false ? (
                                    <div><label className="text-sm font-medium text-gray-700 dark:text-gray-200 mb-1 block">{t("fields.hours")}</label><input type="number" step="0.5" min="0" max="24" value={editingEntry.duration || ''} onChange={(e) => setEditingEntry((prev: any) => ({ ...prev, duration: parseFloat(e.target.value) }))} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-green-500" /></div>
                                ) : (
                                    <>
                                        <div><label className="text-sm font-medium text-gray-700 dark:text-gray-200 mb-1 block">{t("fields.hours")}</label><input type="number" step="0.5" min="0" max="24" value={editingEntry.duration || ''} onChange={(e) => setEditingEntry((prev: any) => ({ ...prev, duration: parseFloat(e.target.value) }))} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-green-500" /></div>
                                        <div><label className="text-sm font-medium text-gray-700 dark:text-gray-200 mb-1 block">{t("fields.kilograms")}</label><input type="number" step="0.1" min="0" value={editingEntry.kilogramsPicked || ''} onChange={(e) => setEditingEntry((prev: any) => ({ ...prev, kilogramsPicked: parseFloat(e.target.value) }))} className="w-full px-3 py-2 border border-orange-300 dark:border-orange-800 dark:bg-gray-700 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-orange-500" /></div>
                                    </>
                                )}
                                <div><label className="text-sm font-medium text-gray-700 dark:text-gray-200 mb-1 block">{t("fields.salary")}</label><p className="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-700/50 text-gray-800 dark:text-gray-100 font-bold">{t("currency.zl", { value: parseFloat(editingEntry.daySalary).toFixed(2) })}</p></div>
                                <div><label className="text-sm font-medium text-gray-700 dark:text-gray-200 mb-1 block">{t("fields.sector")}</label><select value={editingEntry.sector?.id || ''} onChange={(e) => { const sector = sectors.find((s: any) => s.id === parseInt(e.target.value)); setEditingEntry((prev: any) => ({ ...prev, sector: sector || null })); }} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-green-500"><option value="">{t("fields.noSector")}</option>{sectors.map((s: any) => <option key={s.id} value={s.id}>{s.description} ({s.plantType})</option>)}</select></div>
                            </div>
                            <div><label className="text-sm font-medium text-gray-700 dark:text-gray-200 mb-1 block">{t("fields.workType")}</label><select value={editingEntry.workType || ''} onChange={(e) => setEditingEntry((prev: any) => ({ ...prev, workType: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-green-500"><option value="">{t("fields.noSector")}</option>{/* Opcje typów pracy */}</select></div>
