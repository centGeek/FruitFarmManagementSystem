        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-xl font-bold text-gray-900">{t("confirmModal.title")}</h3>
            </div>
          ...
          <div className="space-y-6 mb-6">
            <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
              <h4 className="font-semibold text-blue-900 mb-4">{t("sectorData.groupTitle")}</h4>
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-blue-800 mb-2">{t("sectorData.cropTypeLabel")}</label>
                  <select
                ...
                {editedSector.cropType && selectedCropType && (
                  <div>
                    <label className="block text-sm font-medium text-blue-800 mb-2">{t("sectorData.varietyLabel")}</label>
                    <select
            ...
            {editedSector.cropType && (
              <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                <h4 className="font-semibold text-green-800 mb-2">{t("selectedCrop.title")}</h4>
                <div className="p-3 bg-white rounded border border-green-300">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-green-700">{t("selectedCrop.typeLabel", { type: editedSector.cropType })}</div>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="p-6">
          ...
          <div className="space-y-6 mb-6">
            <div className="bg-orange-50 rounded-lg p-4 border border-orange-200">
              <h4 className="font-semibold text-orange-900 mb-4">{t("sectorData.groupTitle")}</h4>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-orange-800 mb-2">{t("sectorData.nameLabel")}</label>
                  <input
                    type="text"
                    value={editedSector.name}
                    onChange={(e) => handleInputChange('name', e.target.value)}
                ...
                <div>
                  <label className="block text-sm font-medium text-orange-800 mb-2">{t("sectorData.cropTypeLabel")}</label>
                  <select
                    value={editedSector.cropType}
                    onChange={(e) => handleInputChange('cropType', e.target.value)}
                    className="w-full p-3 border border-orange-300 rounded-lg focus:ring-2 focus:ring-orange-500 bg-white"
                ...
                {editedSector.cropType && selectedCropType && (
                  <div>
                    <label className="block text-sm font-medium text-orange-800 mb-2">{t("sectorData.varietyLabel")}</label>
                    <select
                      value={editedSector.variety}
                      onChange={(e) => handleInputChange('variety', e.target.value)}
                      className="w-full p-3 border border-orange-300 rounded-lg focus:ring-2 focus:ring-orange-500 bg-white"
...
export const InteractiveMap = ({
  ...
  <div className="relative">
    ...
    <div className="mb-4 p-4 bg-blue-50 rounded-lg">
      <h3 className="font-semibold text-blue-800 mb-2">{t("map.howTo")}</h3>
    ...
    <div className="absolute bottom-4 left-4 flex flex-col gap-2 z-[1000]">
      ...
      {sectors.length > 0 && (
        <div className="bg-white p-3 rounded-lg shadow-lg text-xs max-w-48">
          <div className="font-semibold text-gray-700 mb-1">{t("map.statistics")}</div>
          <div className="text-gray-600">{t("map.sectorsCount", { count: sectors.length })}</div>
          {editMode !== null && (
  ...
  const renderSectorCard = (sector: Sector, isActive = false) => {
    ...
    return (
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              {t("sectorsList.nameLabel")} {isActive && <span className="text-gray-500">{t("sectorsList.archivedBadge")}</span>}
            </label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("sectorsList.cropTypeLabel")}</label>
            <div className="w-full p-2 border border-gray-200 rounded bg-gray-50 text-gray-800">
        ...
        {sector.variety && (
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-1">{t("sectorsList.varietyLabel")}</label>
                ...
                <div className="space-y-1 font-mono text-xs">
                  {sector.corners.map((corner, idx) => (
                    <div key={idx} className="flex justify-between gap-4">
                      <span className="text-gray-600">{t("sectorsList.point", { index: idx + 1 })}</span>
      ...
      {archivedSectors.length > 0 && (
        <div className="mt-8 pt-6 border-t border-gray-300">
          <h3 className="text-xl font-semibold text-gray-700 mb-4 flex items-center gap-2">
            <Trash2 className="w-5 h-5 text-gray-500" /> {t("sectorsList.archivedTitle", { count: archivedSectors.length })}
          </h3>
