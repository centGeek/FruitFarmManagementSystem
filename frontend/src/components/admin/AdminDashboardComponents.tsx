...
const STATUS_OPTIONS = [
  { value: 'OPEN', labelKey: 'statusOptions.open', icon: '🟢', active: 'bg-blue-600 text-white', idle: 'bg-blue-50 text-blue-700 hover:bg-blue-100' },
  { value: 'IN_PROGRESS', labelKey: 'statusOptions.inProgress', icon: '🛠️', active: 'bg-amber-500 text-white', idle: 'bg-amber-50 text-amber-700 hover:bg-amber-100' },
  { value: 'CLOSED', labelKey: 'statusOptions.closed', icon: '✅', active: 'bg-green-600 text-white', idle: 'bg-green-50 text-green-700 hover:bg-green-100' },
];
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-300">
      <div className="flex items-start justify-between gap-4 mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">#{ticket.id} · {t("ticket.reportedAt", { date: createdAt })}</p>
          </div>
        </div>
      ...
      <div className="pt-3 border-t border-gray-100">
        <p className="text-sm font-medium text-gray-500 uppercase mb-1">{t("ticket.descriptionLabel")}</p>
        <p className="text-base text-gray-900 whitespace-pre-wrap break-words">{ticket.description}</p>
      </div>
      {closedAt && <p className="text-xs text-gray-400 mt-3">{t("ticket.closedAt", { date: closedAt })}</p>}

      <div className="pt-3 mt-3 border-t border-gray-100">
        <p className="text-xs font-medium text-gray-500 uppercase mb-2">{t("ticket.changeStatus")}</p>
        <div className="flex flex-wrap gap-2">
      ...
      <div className="pt-3 mt-3 border-t border-gray-100">
        <p className="text-xs font-medium text-gray-500 uppercase mb-2">{t("ticket.adminCommentLabel")}</p>
        <textarea
          value={comment}
