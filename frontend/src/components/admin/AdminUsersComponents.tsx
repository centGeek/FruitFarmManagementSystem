...
const ROLE_BADGE: Record<string, { className: string; icon: string }> = {
  Admin: { className: 'bg-purple-100 text-purple-700 border-purple-200', icon: '🛠️' },
  Gardener: { className: 'bg-green-100 text-green-700 border-green-200', icon: '🌱' },
  Employee: { className: 'bg-blue-100 text-blue-700 border-blue-200', icon: '👷' },
};
...
export const EmptyState = () => {
  ...
  return (
    <div className="text-center py-16">
      ...
      <p className="text-gray-500 max-w-md mx-auto">{t("empty.subtitle")}</p>
    </div>
  );
};
...
export const RoleBadge = ({ roleName }: { roleName: string }) => {
  const { t } = useTranslation("adminUsers");
  const badge = ROLE_BADGE[roleName] ?? { className: 'bg-gray-100 text-gray-700 border-gray-200', icon: '❔' };
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
        <h3 className="text-xl font-bold text-gray-900 mb-1">{t('resetModal.title')}</h3>
        ...
        <div className="flex justify-end gap-3">
          <button onClick={onClose} className="px-4 py-2 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50">{t('common:actions.cancel')}</button>
    ...
    <div className={`bg-white border rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-300 ${user.active ? 'border-gray-200' : 'border-red-200 bg-red-50/40'}`}>
      <div className="flex items-start justify-between gap-4 mb-3">
        <div>
          ...
          <p className="text-xs text-gray-500">@{user.nickname} · #{user.id} · {t('row.joined', { date: created })}</p>
        </div>
      ...
      {user.roleName === 'Gardener' && (
        <p className="text-sm text-gray-600 mb-3">{t('row.employeeCount')} <strong>{user.employeeCount}</strong></p>
      )}
      {user.gardenerName && (
        <p className="text-sm text-gray-600 mb-3">{t('row.gardener')} <strong>{user.gardenerName}</strong></p>
      )}
      ...
      <div className="pt-3 border-t border-gray-100 flex flex-wrap items-center gap-3">
        ...
        {isAdmin && <span className="text-xs text-gray-400">{t('row.adminLocked')}</span>}
      </div>
