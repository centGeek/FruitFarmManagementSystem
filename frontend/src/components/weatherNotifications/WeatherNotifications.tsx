        ...
        <div className="p-6 max-w-7xl mx-auto">
            <div className="mb-6">
                <h1 className="text-3xl font-bold text-gray-800 mb-2 flex items-center gap-3">
                    <Cloud className="w-8 h-8 text-blue-500" /> {t('page.title')}
                </h1>
            ...
            {isLoadingWeather && coordinates === null && weatherError === null ? (
                        ...
                        <div>
                            ...
                            {missingLocation && (
                                <p className="text-sm text-red-700 mt-1">
                                    {t('weather.missingLocationPrefix')}<Link to="/gardener-profile" className="font-bold underline text-red-800 hover:text-red-900">{t('weather.profileSettings')}</Link>{t('weather.missingLocationSuffix')}
                                </p>
                            )}
                        </div>
                    </div>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-8 mb-6">
                    <div className="flex items-center justify-center gap-3">
                        <Loader className="w-6 h-6 text-blue-600 animate-spin" />
                        <span className="text-blue-800 font-medium">{t('weather.fetching', { location: locationName })}</span>
                    </div>
                </div>
            )}
            ...
            {isCheckingAlerts ? (
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 mb-6">
                    <div className="flex items-center justify-center gap-3">
                        ...
                        <span className="text-amber-800 font-medium">{t('alerts.checking')}</span>
                    </div>
                </div>
                ...
                <div className="mb-6 space-y-3">
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <h3 className="text-lg font-semibold text-gray-800">{t('alerts.heading', { count: forecastAlerts.length })}</h3>
                    </div>
                                ...
                                <div className="flex items-start justify-between gap-4">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <span className="text-sm text-gray-600">{new Date(alert.date).toLocaleDateString(i18n.language?.startsWith('en') ? 'en-US' : 'pl-PL', { weekday: 'long', day: 'numeric', month: 'long' })}</span>
                                        </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
                <div className="flex items-center justify-between mb-6">
                    <h2 className="text-2xl font-semibold text-gray-800 flex items-center gap-2"><Bell className="w-6 h-6 text-blue-500" /> {t('rules.title')}</h2>
                ...
                {isLoadingNotifications ? (
                    <div className="text-center py-12">
                        ...
                        <p className="text-gray-600">{t('rules.loading')}</p>
                    </div>
                ) : notifications.length === 0 ? (
                    <div className="text-center py-12 bg-gray-50 rounded-lg">
                        <Bell className="w-16 h-16 text-gray-400 mx-auto mb-4" />
                        <p className="text-xl text-gray-500 mb-2">{t('rules.empty')}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
                ...
                <div className="bg-amber-50 p-4 rounded-lg border border-amber-200">
                    ...
                    <div className="text-amber-800">{t('stats.types')}</div>
                </div>
            </div>
