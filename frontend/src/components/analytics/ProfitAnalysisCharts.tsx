...
const PieTooltip = ({ active, payload }: any) => {
    ...
    if (active && payload && payload.length) {
        ...
        return (
            <div className="bg-white p-4 border border-gray-300 rounded-lg shadow-lg">
                <p className="font-bold text-gray-800 mb-2">{displayName(data.name, t)}</p>
                <p className="text-sm font-medium text-green-600">{t("tooltip.revenue")}: {formatCurrency(data.revenue)} PLN</p>
                <p className="text-sm font-medium text-red-600">{t("tooltip.expenses")}: {formatCurrency(data.expenses)} PLN</p>
                <p className="text-sm font-medium text-blue-600">{t("tooltip.profit")}: {formatCurrency(data.profit)} PLN</p>
            </div>
        );
    }
    return null;
...
const VarietyTooltip = ({ active, payload }: any) => {
    ...
    if (active && payload && payload.length) {
        ...
        return (
            <div className="bg-white p-4 border border-gray-300 rounded-lg shadow-lg">
                <p className="font-bold text-gray-800 mb-2">{varietyName(data.name, t)}</p>
                <p className="text-sm font-medium text-green-600">{t("tooltip.revenueSingular")}: {formatCurrency(data.revenue)} PLN</p>
                <p className="text-sm font-medium text-blue-600">{t("tooltip.soldKg", { kilograms: data.kilograms })}</p>
                {data.revenue > 0 && data.kilograms > 0 && (
                    <p className="text-sm font-medium text-purple-600">{t("tooltip.pricePerKg")}: {formatCurrency(data.revenue / data.kilograms)} PLN</p>
                )}
            </div>
        );
    }
        ...
        if (active && payload && payload.length) {
            ...
            return (
                <div className="bg-white p-4 border border-gray-300 rounded-lg shadow-lg">
                    ...
                    <p className="text-sm font-medium text-purple-600">{t("tooltip.totalCost")}: {formatCurrency(d.value)} PLN</p>
                    {d.paidCost > 0 && <p className="text-sm font-medium text-green-600">{t("tooltip.paid")}: {formatCurrency(d.paidCost)} PLN</p>}
                    {d.unpaidCost > 0 && <p className="text-sm font-medium text-amber-600">{t("tooltip.toPay")}: {formatCurrency(d.unpaidCost)} PLN</p>}
                    {d.entries > 0 && <p className="text-sm font-medium text-gray-600">{t("tooltip.entriesCount", { count: d.entries })}</p>}
                </div>
            );
        }
        return null;
                ...
                {data.map((row: any, index: number) => {
                    ...
                    return (
                        <tr key={index} className={`border-b border-gray-200 hover:bg-gray-50 transition-colors ${index === 0 ? 'bg-green-50' : index === data.length - 1 ? 'bg-red-50' : ''}`}>
                            ...
                            <td className="py-4 px-4"><span className="font-semibold text-gray-900">{displayName(row.name, t)}</span></td>
